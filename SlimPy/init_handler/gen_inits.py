...
def find_by_default_window(specaxis,specdata,init_params,window_lines,verbose=0):
    ...
    max_I_names = ["o_iii_2","s_iv_2","n_iv","ne_viii","o_iv","n_iii_2","o_iii_2","c_iii","o_vi","h_i_be","ne_viii","h_i_ga"]
    ...
    for  i in range(len(default_windows)):
        if lines_names_sorted == default_windows[i]:
            ...
            if "ne_viii" in lines_names_sorted:
                ne_viii_wvl = 770.42
            ...
            # print("lines_names_sorted",lines_names_sorted,"default_windows[i]",default_windows[i])
...
def gen_fit_inits(RasterOrPath,
    ...
    if True: #This part should be stored and re-imported 
        default_lines = [
            ...
            {"name":"n_iv"      ,"wvl": 765.15},
            {"name":"ne_viii"   ,"wvl": 770.42},
            {"name":"h_i_ga"    ,"wvl": 969.00},#972.54},
            {"name":"o_1&ne_vii","wvl": 972.54},#973.60},
            {"name":"c_iii"     ,"wvl": 977.03},
            {"name":"s_v"       ,"wvl": 786.47},
            {"name":"o_iv"      ,"wvl": 787.72},
            {"name":"o_i"       ,"wvl": 988.75},
    ...
    for kw_i, kw in enumerate(unq):
        ...
        for line in default_lines:
            if np.min(specaxis)-1<=line["wvl"]<=np.max(specaxis)+1:
        ...
        if verbose >= 4:
            ...
            plt.legend()
            plt.savefig(f"./tmp/window{kw_i}_{dtime}.jpg")
    ...
    if verbose >= 3:
        ...
        fig1.savefig(f"./tmp/window_all_{dtime}.jpg")
        fig2.savefig(f"./tmp/spectrum_all_{dtime}.jpg")
